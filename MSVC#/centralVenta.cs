...
namespace MundoMusical.VENTA
{
    public partial class centralVenta : BaseForm
    {
        ...
        private void refresh_inventario_temporal() //REFRESCA INVENTARIO TEMPORAL AGREGADO 07/05/2019
        {
            this.inventario = this.db.getinventarioproductos();

            foreach(DataGridViewRow dr in this.dgv.Rows)
            {
                //genericDefinitions.ok(this.inventario.Find(x => x.idproducto.ToString() == dr.Cells[0].Value.ToString()).stock.ToString());
                this.inventario.Find(x => x.idproducto.ToString() == dr.Cells[0].Value.ToString()).stock -= int.Parse(dr.Cells[2].Value.ToString());
                //genericDefinitions.ok(this.inventario.Find(x => x.idproducto.ToString() == dr.Cells[0].Value.ToString()).stock.ToString());
            }
        }
        ...
        {
            refresh_inventario_temporal(); //REFRESCA INVENTARIO TEMPORAL AGREGADO 07/05/2019
        ...
        private bool codetxtproducto() //VERIFICA LA INTEGRIDAD DE LOS DATOS DE ITEM ANTES DE AGREGARLOS AL DGV (DESPUES DE ENTER EN TXTIDPRODUCTO Y CLICK EN BTN AGREGAR PRODUCTO)
        {
            ...
            */
               ...
               {
                    genericDefinitions.dangerInfo("No existe existencia de este producto en la base de datos Temporal", "Error");
